...
<div class="order-form-section" id="order-form-section">
  <div class="order-form-box">
    ...
    <div class="form-group">
      ...
      <input type="text" id="f-direccion" placeholder="Entregar a domicilio o dejar en oficina Interrapidísimo">
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <textarea id="f-nota" placeholder="Observaciones, color, talla, instrucciones especiales..."></textarea>
    </div>
...
function confirmOrder(){
  ...
  if(!nombre||!apellido||!ciudad||!direccion||!telefono){alert('Por favor completa todos los campos obligatorios.');return;}
  const p=currentOrderProduct;
  const order={id:'ORD'+Date.now(),product:p.name,productId:p.id,price:p.price,oldPrice:p.oldPrice,nombre,apellido,ciudad,direccion,telefono,nota,fecha:new Date().toLocaleString('es-CO')};
  orders.push(order);
  localStorage.setItem('trogui_orders',JSON.stringify(orders));
  const msg=encodeURIComponent(`🛍️ *NUEVO PEDIDO - TROGÜI*\n\n📦 *Producto:* ${p.name}\n🆔 *ID:* ${p.id}\n💰 *Precio:* $${fmt(p.price)}\n~~Antes: $${fmt(p.oldPrice)}~~\n\n👤 *Cliente:* ${nombre} ${apellido}\n🏙️ *Ciudad:* ${ciudad}\n🏠 *Dirección:* ${direccion}\n📞 *Teléfono:* ${telefono}\n📝 *Nota:* ${nota||'Sin nota'}\n\n✅ Pago contra entrega`);
  closeOrderForm();
...
function renderAdminProducts(){
  ...
  list.innerHTML=products.map((p,i)=>`
    <div class="admin-product-item" id="aitem-${p.id}">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <b style="font-size:14px;color:var(--orange)">${p.id} - ${p.name}</b>
        <button class="btn-del-admin" onclick="deleteProduct('${p.id}')">🗑️ Eliminar</button>
      </div>
      <label>Nombre</label>
      <input type="text" id="an-${p.id}" value="${p.name}">
      <label>Descripción</label>
      <textarea id="ad-${p.id}" rows="2">${p.desc}</textarea>
      <div class="row2">
        <div><label>Precio actual ($)</label><input type="number" id="ap-${p.id}" value="${p.price}"></div>
      ...
      <input type="text" id="ai-${p.id}" value="${p.img}" placeholder="URL de imagen o pega link">
      <label>URL Imagen Secundaria</label>
      <input type="text" id="ai2-${p.id}" value="${p.img2||p.img}" placeholder="URL segunda imagen">
      <label>Categoría</label>
      <select id="ac-${p.id}" style="width:100%;padding:7px;border:1.5px solid var(--border);border-radius:7px;font-size:13px">
        ${['belleza','hogar','tecnologia','salud','fitness','juguetes','accesorios'].map(c=>`<option value="${c}"${p.cat===c?' selected':''}>${c}</option>`).join('')}
      </select>
      <div class="row2">
        <div><label>Últimas unidades</label><select id="alu-${p.id}" style="width:100%;padding:7px;border:1.5px solid var(--border);border-radius:7px;font-size:13px"><option value="true"${p.lastUnits?' selected':''}>Sí</option><option value="false"${!p.lastUnits?' selected':''}>No</option></select></div>
...
function saveProducts(silent){
  products.forEach(p=>{
    const n=document.getElementById('an-'+p.id);if(!n)return;
    p.name=n.value;
    p.desc=document.getElementById('ad-'+p.id).value;
    p.price=parseInt(document.getElementById('ap-'+p.id).value)||p.price;
    p.oldPrice=parseInt(document.getElementById('ao-'+p.id).value)||p.oldPrice;
    p.sold=parseInt(document.getElementById('av-'+p.id).value)||p.sold;
    p.stars=parseInt(document.getElementById('as-'+p.id).value)||p.stars;
    p.img=document.getElementById('ai-'+p.id).value||p.img;
    p.img2=document.getElementById('ai2-'+p.id).value||p.img2;
    p.cat=document.getElementById('ac-'+p.id).value;
    p.lastUnits=document.getElementById('alu-'+p.id).value==='true';
    p.timer=parseInt(document.getElementById('at-'+p.id).value)||p.timer;
  });
  localStorage.setItem('trogui_products',JSON.stringify(products));
  renderProducts(products);
  if(!silent) alert('✅ Productos guardados correctamente!');
}

// ===================== ADMIN C - ORDERS =====================
function openAdminC(){
  const list=document.getElementById('admin-orders-list');
  if(orders.length===0){list.innerHTML='<p style="color:var(--gray);text-align:center;padding:20px">No hay pedidos aún.</p>';}
  else{
    list.innerHTML=orders.map(o=>`
      <div class="order-item">
        <strong>#${o.id}</strong> - ${o.product} (ID: ${o.productId})<br>
        👤 ${o.nombre} ${o.apellido} · 📞 ${o.telefono}<br>
        🏙️ ${o.ciudad} · 🏠 ${o.direccion}<br>
        💰 Precio: $${fmt(o.price)} <span style="text-decoration:line-through;color:var(--gray);font-size:12px">antes $${fmt(o.oldPrice)}</span><br>
        ${o.nota?'📝 '+o.nota+'<br>':''}
        <div class="order-meta">📅 ${o.fecha}</div>
      </div>`).join('');
  }
